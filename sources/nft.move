module nft:: nft {   //defines a new module named nft
    ...
    public struct DavidxmNFT has key, store { //defines a struct with the key and store abilities. 
    ...
    fun init(witness: NFT, ctx: &mut TxContext) {
        ...
        let values = vector[
           ...
           // string::utf8(b"{url}"),
            string::utf8(b"{image_url}"),
            string::utf8(b"DavidxmNFT")
        ];

        let mut display = display::new_with_fields<DavidxmNFT>(
            &publisher, 
            keys,
            values,
            ctx
    ...
    public entry fun burn(nft: DavidxmNFT) {
        let DavidxmNFT { id, name: _, description: _, url: _ } = nft;
        object::delete(id)
    }

    /// Get the NFT's name
    public fun name(nft: &DavidxmNFT): &string::String {
        &nft.name
    }

    /// Get the NFT's description
    public fun description(nft: &DavidxmNFT): &string::String {
        &nft.description
    }

    /// Get the NFT's url
    public fun url(nft: &DavidxmNFT): &Url {
        &nft.url
    }
}